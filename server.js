...
app.post('/quiz', (req, res) => { // post because get won't work with Canvas
	...
	const course = passed.course;
	const qbankAndNumArray = passed.qbankAndNumArray;
	const date = encodeURIComponent(String(passed.date).replace(/,/g, '').replace(/ /g, '_'));
	//const date = encodeURIComponent(passed.date.toString().replaceAll(',', '').replaceAll(' ', '_'));
	const title = passed.title;
...
if(selected) {
  ...
  productFile = productFile.replace(/(?<!let )(questionsObject = .*?;)/, 'const fileName = "' + fileName + '"; ' + 'const sessionId =' + JSON.stringify(sessionId) + '; ' + 'const courseId = "' + courseId + '"; ' + 'const assignmentId = "' + assignmentId + '"; ' + 'const studentId = "' + studentId + '";' + ' selectedObjText = ' + selectedObj + '; ' + '// Error:' + errorVar )  
